...
import DashboardSkeleton from "../../skeleton/user/DashboardSkeleton.jsx";
import InvoiceStore from "../../store/InvoiceStore.js";

const Dashboard = () => {
    const {ProfileDetails, UserDetails} = UserStore()
    ...
    if (InvoiceList === null || loading || ProfileDetails === {}) {
        return <DashboardSkeleton />
    }else{
        return (
            <div className="bg-bg-secondary w-full md:w-3/4 p-6">
                {/* Account Information */}
                <div className="bg-white p-6 mb-6 shadow-md rounded">
                    <h2 className="text-black text-xl font-bold mb-4">Account Information</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <p>
                                <span
                                    className="text-black font-semibold">Name:</span> {ProfileDetails ? ProfileDetails.cus_name : ""}
                            </p>
                            <p>
                                <span
                                    className="text-black font-semibold">Email:</span> {UserDetails.email}
                            </p>
                            <p>
                                <span
                                    className="text-black font-semibold">Phone:</span> +88 {ProfileDetails ? ProfileDetails.cus_phone : ""}
                            </p>
                        </div>
                        <div>
                            <p>
                                <span
                                    className="text-black font-semibold">City:</span> {ProfileDetails ? ProfileDetails.cus_city : ""}
                            </p>
                            <p>
                                <span
                                    className="text-black font-semibold">State:</span> {ProfileDetails ? ProfileDetails.cus_state : ""}
                            </p>
                            <p>
                                <span
                                    className="text-black font-semibold">Country:</span> {ProfileDetails ? ProfileDetails.cus_country : ""}
                            </p>
                        </div>
