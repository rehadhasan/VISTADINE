                ...
                {MenuProductList === null ? (
                    ...
                    <>
                        ...
                        <div className="flex justify-center mb-6">
                            <button
                                onClick={async () => {await handleButtonClick('All'); await MenuProductListRequest()}}
                            ...
                            <button
                                onClick={async () => {await handleButtonClick('Family'); await MenuProductListByRemarkRequest('family')}}
                                className={`mx-2 text-lg font-medium py-2 px-4 rounded-full ${
                            ...
                            <button
                                onClick={async () => {await handleButtonClick('Individual'); await MenuProductListByRemarkRequest('individual')}}
                                className={`mx-2 text-lg font-medium py-2 px-4 rounded-full ${
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {MenuProductList.map((item, i) => (
                                <a href={`/item/details/${item._id}`} key={i} className="border p-4 rounded-lg shadow-lg relative">
                                    {
                                        item.remark === 'special'?(
                                            <div className="absolute top-0 left-0 bg-primary text-white text-xs px-2 py-1">SPECIAL</div>
                                        ):('')
                                    }
                        ...
                        <div className="flex justify-center mt-8">
                            <a
                                href={'/menu'}
                                className="border border-primary hover:bg-primary text-primary hover:text-white py-2 px-4 rounded-lg">View
                                All Items
                            </a>
                        </div>
                    </>
